...
import { ArrowLeft, Eye, MessageSquare, Pencil, ThumbsDown, ThumbsUp, Trash2 } from "lucide-react";
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-xl border border-brand-line bg-white p-6 shadow-xl">
        <h2 id="delete-post-title" className="text-xl font-black text-brand-ink">
          게시글을 삭제할까요?
        </h2>
...
export function PostDetail({ postId }: PostDetailProps) {
  ...
  const handleReaction = async (reactionType: CommunityReactionType) => {
    ...
    const response = await fetch(`/api/community/free/posts/${post.id}/reaction`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${auth.token}`,
  ...
  const handleCommentSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(`/api/community/free/posts/${post.id}/comments`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${auth.token}`,
        "Content-Type": "application/json"
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    const response = await fetch(`/api/community/free/comments/${commentId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${auth.token}`
      }
  ...
  const handleDeletePost = async () => {
    ...
    const response = await fetch(`/api/community/free/posts/${post.id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${auth.token}`
      }
