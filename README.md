...
- 로그인과 회원가입, 내 정보 수정의 비밀번호 입력창에 보기/숨기기 버튼을 추가했습니다.
- 로그인 성공 후 `redirect` 값이 있으면 해당 내부 경로로 이동하고, 없으면 메인 화면으로 이동합니다.
- 커뮤니티 목록 API의 기본 페이지 크기는 20개입니다.
- `/mypage/profile`에서 아이디 변경, 비밀번호 변경, 선택과목 수정을 한 번에 관리합니다.
- 기존 `/mypage/subjects`는 `/mypage/profile`로 이동합니다.
- 단체소개 하위 메뉴에 `/vision`, `/symbol` 페이지를 추가했습니다.
- 상단바는 비로그인 상태에서 로그인 텍스트만, 로그인 상태에서 사용자 아이콘과 이름을 하나의 마이페이지 링크로 표시합니다.
- 정시파출소는 다른 페이지와 같은 흰색 본문 제목 영역 구조로 정리했습니다.
